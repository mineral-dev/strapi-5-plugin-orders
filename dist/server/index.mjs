...
const controller = ({ strapi }) => ({
  index(ctx) {
    ctx.body = strapi.plugin("strapi-5-orders").service("service").getWelcomeMessage();
  },
  async getOrder(ctx) {
    const { page, status, q } = ctx.request.query;
    try {
      ctx.body = await strapi.plugin("strapi-5-orders").service("service").getOrders({ page, status, q });
    } catch (error) {
  ...
  async getOrderById(ctx) {
    const { id } = ctx.params;
    try {
      ctx.body = await strapi.plugin("strapi-5-orders").service("service").getOrderById(id);
    } catch (error) {
      ctx.send({ message: err.message }, 409);
    }
  }
